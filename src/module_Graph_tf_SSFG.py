...
from chemutils import vocabulary1 as vocabulary
from chemutils import vocabulary_weight
from Multihead_module import multihead_layer

class Graph_model(nn.Module):

    def __init__(self,nfeat, nhid, num_head, num_multi_layers,num_rel_gcn_layers,num_edge_types,num_linear_layer):
        ...
        for i in range(self.num_multi_layers):
            self.multi_layers_lst.append(multihead_layer(self.num_head,self.nfeat,self.nhid[0]))
        ...
        for i in range(self.num_rel_gcn_layers):
            if i < self.num_rel_gcn_layers - 1:
                ...
                self.rgc_seq.add_module(name=f'rgc_{i}', module=dglnn.RelGraphConv(nhid[i], nhid[i], num_edge_types,
                                                                                   ...
                                                                                   self_loop=True,dropout=0.1,
                                                                                   layer_norm=True))
        self.total_skip_ln = nn.Linear(nfeat*(self.num_multi_layers+1),self.nhid[0],bias=False)
        self.total_skip_norm = nn.LayerNorm(self.nhid[0])
        ...
        for i in range(self.num_linear_layer):
            ...
            self.LN_seq.add_module(name=f'LN_{i}', module=nn.LayerNorm(linear_nhid[i]))
        self.out_fc = nn.Linear(linear_nhid[-1], self.vocabulary_size)
        self.opt = torch.optim.SGD(self.parameters(), lr=1e-2)
        # weight =
        self.criteria = torch.nn.CrossEntropyLoss(ignore_index=self.vocabulary_size,weight=torch.tensor(vocabulary_weight,dtype=torch.float))
    ...
    def forward(self,graph:dgl.DGLGraph):
        ...
        for multi_att_layer in self.multi_layers_lst:
            x_node = multi_att_layer(graph,x_node,x_edge)
            # x_node = ssfg.apply(x_node,self.training)
            skip_mulatt.append(x_node)
        ...
        for gc in self.rgc_seq:
            ...
            # total_skips = ssfg.apply(total_skips,self.training)
    ...
    def infer(self, graph, label, mask_idx):
        pred_y = self.forward(graph)
